...
export const Tool = ({ className, ...props }: ToolProps) => (
  <Collapsible
    className={cn("not-prose mb-0 w-full rounded-lg border bg-card", className)}
    {...props}
  />
);
...
export const ToolOutput = ({
  ...
  let rendered: ReactNode = <div>{output as ReactNode}</div>;

  if (typeof output === "string") {
    rendered = <pre className="whitespace-pre-wrap p-3 font-mono">{output}</pre>;
  ...
  return (
    <div className={cn("space-y-2 p-4", className)} {...props}>
      ...
      <div
        className={cn(
          "max-h-72 overflow-auto rounded-md text-xs",
          errorText
            ? "bg-destructive/10 text-destructive"
            : "bg-muted/50 text-foreground"
        )}
      >
        {errorText ? <div className="p-3">{errorText}</div> : rendered}
      </div>
    </div>
  );
};
